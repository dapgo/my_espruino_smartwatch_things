...
if (v_model=='BANGLEJS'||v_model=='EMSCRIPTEN') {
 ...
 if (v_mode_debug>0) {
    ...
    function ClearActiveArea(){        
        //use default g.setColor(v_color_b_area); 
...
*/  
    ...
    function PrintHelloWorld(){     
        ...
        g.drawString("Display count: "+contador ,10,box_y2+5); 
        mem=process.memory();
        //console.log("Mem free/total: "+mem.free+"/"+mem.total);    
        g.drawString("Free mem: "+mem.free+"/"+mem.total,10,box_y2+5+v_font3size); 
        g.flip();    
    }
    ...
    function PrintMainStaticArea(){ 
        ...
        g.setFont("Vector",v_font3size);    
        g.drawString("#by DPG #bangle.js",10,box_y2+5+(v_font3size*3)); 
        g.drawString("#javascript #espruino",10,box_y2+5+(v_font3size*4)); 
   ...
   if (v_model=='BANGLEJS'||v_model=='EMSCRIPTEN') {
	...
	// else setWatch(, BTN1, { repeat: true });//func  to quit
        Bangle.on('swipe', dir => {  
